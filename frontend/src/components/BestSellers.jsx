...
export default function BestSellers() {
  ...
  const eyebrow = content?.eyebrow || 'Loved by our customers';
  const title = content?.title || 'Best Sellers';
  const viewAllText = content?.viewAllText || 'View More';
  const viewAllLink = content?.viewAllLink || '/products?sort=best-selling';
  const maxItems = content?.maxItems || 8;

  const scrollContainerRef = useRef(null);
  const bestSellers = products.filter(p => p.isBestSeller).slice(0, maxItems);
  ...
  return (
    <section ref={sectionRef} className="pt-2 pb-1.5 sm:pt-4 sm:pb-3 lg:pt-5 lg:pb-3.5 bg-brand-cream/30 border-b border-brand-powder/30 relative overflow-hidden">
