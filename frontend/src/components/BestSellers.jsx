...
export default function BestSellers() {
  ...
  const content = getSectionContent('best-sellers');

  const eyebrow = content?.eyebrow || 'Most Loved';
  const title = content?.title || 'Best Sellers';
  const viewAllText = content?.viewAllText || 'View All';
  const viewAllLink = content?.viewAllLink || '/products?sort=best-selling';
  const maxItems = content?.maxItems || 12;

  const scrollContainerRef = useRef(null);

  // Ensure plenty of items so slider is always scrollable
  const displayItems = React.useMemo(() => {
    ...
    const bestItems = list.filter((p) => p.isBestSeller);
    const otherItems = list.filter((p) => !p.isBestSeller);
    const combined = [...bestItems, ...otherItems];
  ...
  return (
    <section ref={sectionRef} className="pt-2 pb-1.5 sm:pt-4 sm:pb-3 lg:pt-5 lg:pb-3.5 bg-brand-cream/30 border-b border-brand-powder/30 relative overflow-hidden group/section">
